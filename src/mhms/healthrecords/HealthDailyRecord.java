...
public class HealthDailyRecord extends TabActivity {
	...
	class EditTextListener2 implements TextWatcher {
		public void afterTextChanged(Editable s) {
			...
			Cursor cursor = db.query("mhmslog", new String[] { "id,systemtime,log" }, "log like ?",
					new String[] { logstring }, null, null, null, null);
			...
			SimpleAdapter adapter = new SimpleAdapter(HealthDailyRecord.this, lists, R.layout.search_show_list,
					new String[] { "id", "systemtime", "log" },
	...
	private void healthdailyrecord_select_show() {
		...
		SQLiteDatabase db = dbHelper.getReadableDatabase();
		List<HashMap<String, Object>> lists = new ArrayList<HashMap<String, Object>>();
		Cursor cursor = db.query("mhmslog", new String[] { "id,systemtime,log" }, "systemtime like ?",
				new String[] { dateSelect }, null, null, null, null);
		while (cursor.moveToNext()) {
			HashMap<String, Object> listmap = new HashMap<String, Object>();
			listmap.put("id", cursor.getString(cursor.getColumnIndex("id")));
			listmap.put("systemtime", cursor.getString(cursor.getColumnIndex("systemtime")));
