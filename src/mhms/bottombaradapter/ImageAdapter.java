...
public class ImageAdapter extends BaseAdapter {
    ...
    public ImageAdapter(Context c,int[] picIds,int width,int height,int selResId) { 
        ...
        for(int i=0;i<picIds.length;i++)
        {
        	...
        	imgItems[i].setLayoutParams(new GridView.LayoutParams(width, height));//set the wide,height of ImageView
    ...
    {  
        ...
        {  
            if(i!=index)  
            {  
            	imgItems[i].setBackgroundResource(0);//back to the form not chosen
            }  
        }  
